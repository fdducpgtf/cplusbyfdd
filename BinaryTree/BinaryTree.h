...
public:
	...
	void createBTreeFromPreAndInOrder(vector<T> preOrder, vector<T> inOrder, BiTNode<T> *root);
...
template<typename T>
void BinaryTree<T>::createBTreeFromPreAndInOrder(vector<T> preOrder, vector<T> inOrder, BiTNode<T> *root) {
	//如果preOrder为空就退出
	if (preOrder.size()) {
		...
		if (preOrder.size()) {
			...
			if (leftPreOrder.size()) {
				...
				createBTreeFromPreAndInOrder(leftPreOrder, leftInOrder, root->Lchild);
			}
			...
			if (rightPreOrder.size()) {
				...
				createBTreeFromPreAndInOrder(rightPreOrder, rightInOrder, root->Rchild);
			}
